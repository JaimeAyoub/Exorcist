    ...
    out float2 vertex1, out float2 vertex2, out float2 vertex3)
{
    ...
    // Seleccion sin ramas (evita mezclar if/else con ddx/ddy, que en
    // algunos compiladores D3D11 genera errores de inicializacion
    // "fantasma" en funciones no relacionadas mas adelante en el shader)
    float upper = step(0.0, temp.z); // 1.0 si temp.z > 0, si no 0.0
    float lower = 1.0 - upper;

    w1 = upper * temp.z + lower * (-temp.z);
    w2 = upper * temp.y + lower * (1.0 - temp.y);
    w3 = upper * temp.x + lower * (1.0 - temp.x);

    vertex1 = baseId + lower * float2(1.0, 1.0);
    vertex2 = baseId + float2(0.0, 1.0) * upper + float2(1.0, 0.0) * lower;
    vertex3 = baseId + float2(1.0, 0.0) * upper + float2(0.0, 1.0) * lower;
}
